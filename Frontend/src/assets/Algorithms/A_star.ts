...
export function a_star(data, startNode, endNode) {
  if (!startNode) {
  ...
  while (unvisitedNodes.length) {
    ...
    const h1 = closestNode.row-endNode.row;
    const h2 = closestNode.col-endNode.col;
    updateUnvisitedNeighbors(closestNode,h1,h2, data);
  }
}

function animateA_Star(visitedNodesInOrder: node[]) {
  console.log('animate');
...
function updateUnvisitedNeighbors(node,h1,h2, data) {
  const unvisitedNeighbors = getUnvisitedNeighbors(node, data);
  for (const neighbor of unvisitedNeighbors) {
    neighbor.distance = node.distance + 1 + Math.abs(h1) + Math.abs(h2);
